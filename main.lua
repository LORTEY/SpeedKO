...
function speedko:mapWholePage(debug)
    ...
    if not word or not word.pos0 then
        ...
        local ret = self:fallbackPageMap(res)
...
function speedko:fallbackPageMap(res) --used when xpointers are not present
    local lines = {}
    logger.dbg(dump(res))
    ...
    for _, line in ipairs(lines) do
        local line_map = self:recursiveLineMap(line)
        logger.dbg("Lortey LINE MAPPED " .. dump(line_map))
        local NextWord = {
            box = { {} },
            text = {},
        }
    ...
    local result = {}
    if true then
        for _, subtable in ipairs(words) do
            if subtable.text then -- Check if "a" exists
                table.insert(result, dump(subtable.text)) -- Add to result list
            end
        end
